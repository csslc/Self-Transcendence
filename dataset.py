...
class CustomDataset_512(Dataset):
    def __init__(self, data_dir):
        PIL.Image.init()
        supported_ext = PIL.Image.EXTENSION.keys() | {'.npy'}

        self.images_dir = os.path.join(data_dir, 'images-512')
    ...
    def __getitem__(self, idx):
        ...
        feature_path = os.path.join(self.features_dir, feature_fname)

        try:
            features = np.load(feature_path)
        except EOFError:
            print(f"[EOFError] Empty or corrupted file: {feature_path}")
            raise
        except Exception as e:
            print(f"[Error] Failed to load feature file {feature_path}: {e}")
            raise
...
class MSCOCOFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # NEW: read the k-th line text from {index}.txt
        txt_path = os.path.join(self.root, f'{index}.txt')
        with open(txt_path, 'r', encoding='utf-8') as f:
            lines = f.read().splitlines()  # 去掉每行末尾的 \n
        if k >= len(lines):
            raise IndexError(
                f"Index {index}: k={k} but {txt_path} only has {len(lines)} lines."
            )

        t = lines[k]  # 第k行（0-based）
...
class MSCOCO256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    ...
    @property
    def fid_stat(self):
        return f'.../fid_stats/fid_stats_mscoco256_val.npz'