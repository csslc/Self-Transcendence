...
def main(args):
    ...
    folder_name = f"{model_string_name}-{ckpt_string_name}-size-{args.resolution}-vae-{args.vae}-" \
                  f"cfg-{args.cfg_scale}--guidance-{args.guidance_high}-{args.guidance_low}seed-{args.global_seed}-{args.mode}"
    ...
    for i in pbar:
        ...
        # y = torch.ones(n, device=device, dtype=torch.long) * 49
        # y = torch.ones(n, device=device) * 
...
if __name__ == "__main__":
    ...
    # logging/saving:
    parser.add_argument("--ckpt", type=str, default="xxx.pt", help="Optional path to a SiT checkpoint.")
