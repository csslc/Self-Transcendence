...
def main(args):
    ...
    # Setup data:
    all_dataset = MSCOCO256Features(path='.../coco256_features', mode='val')
    ...
    txt_folder_dir = f"{sample_folder_dir}_txt"
    if accelerator.is_main_process:
        os.makedirs(sample_folder_dir, exist_ok=True)
        os.makedirs(real_sample_folder_dir, exist_ok=True)
        os.makedirs(txt_folder_dir, exist_ok=True)
    ...
    for raw_image, _, context, txt in val_dataloader:
        # Sample inputs:
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(samples):
                ...
                Image.fromarray(real_samples[i]).save(f"{real_sample_folder_dir}/{index:06d}.png")

                # NEW: save txt
                line = _to_str(txt[i] if isinstance(txt, (list, tuple)) else txt)
                with open(f"{txt_folder_dir}/{index:06d}.txt", "w", encoding="utf-8") as f:
                    f.write(line.rstrip("\n") + "\n")
