...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        self.projectors = build_mlp(hidden_size, projector_dim, hidden_size)
    ...
    def forward_core_with_concat(
            ...
            detach: Optional[bool] = False, return_fea=None, return_proj_fea=None) -> torch.Tensor:
        ...
        for i, block in enumerate(self.joint_blocks):
            ...
            if return_fea is not None:
                if (i + 1) == return_fea:
                    return x, None
            if return_proj_fea is not None:
                if (i + 1) == return_proj_fea:
                    x_fea = self.projectors(x.reshape(-1, D)).reshape(B, L, -1)
    ...
    def forward(
            self, x: torch.Tensor, t: torch.Tensor, y: Optional[torch.Tensor] = None, context: Optional[torch.Tensor] = None, return_feaOptional[torch.Tensor]=None, return_proj_feaOptional[torch.Tensor]=None,
        ...
        x, x_fea = self.forward_core_with_concat(x, c, context, detach, return_fea, return_proj_fea)
