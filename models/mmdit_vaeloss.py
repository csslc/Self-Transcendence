...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        self.projectors_final = FinalLayer(hidden_size, patch_size, self.out_channels, dtype=dtype, device=device)
    ...
    def forward_core_with_concat(
        ...
        x_fea = self.projectors(out_fea.reshape(-1, D)).reshape(B, L, -1)
        x_fea = self.projectors_final(x_fea, c_mod)
    ...
    def forward(
        ...
        x, x_fea = self.forward_core_with_concat(x, c, context, detach)

        x_fea = self.unpatchify(x_fea,  hw=hw)
