...
class SiT(nn.Module):
    ...
    def patchify(self, imgs):
        """
        imgs: (N, C, H, W)
        returns: (N, T, patch_size**2 * C)
        """
        p = self.x_embedder.patch_size[0]  # patch size
        ...
        # Reshape and permute
        x = imgs.reshape(shape=(imgs.shape[0], imgs.shape[1], h, p, w, p))  # (N, C, h, p, w, p)
        x = torch.einsum('nchpwq->nhwpqc', x)  # (N, h, w, p, p, C)
        x = x.reshape(shape=(imgs.shape[0], h * w, p * p * imgs.shape[1]))  # (N, T, patch_size**2 * C)
        return x
    
    
    def forward(self, x, t, y, return_fea=None, return_proj_fea=None):
        """
        Forward pass of SiT.
