...
We follow the preprocessing code used in [REPA](https://github.com/sihyun-yu/REPA/tree/main/preprocessing) and [U-ViT](https://github.com/baofff/U-ViT). In this code we made a several edits: (1) we removed unncessary parts except preprocessing because this code is only used for preprocessing, (2) we use [-1, 1] range for an input to the stable diffusion VAE (similar to DiT or SiT) unlike edm2 that uses [0, 1] range, and (3) we consider preprocessing to 256x256 resolution (or 512x512 resolution).
...
For processing the T2I dataset (MSCoCo), you can refer to the pipeline in [U-ViT](https://github.com/baofff/U-ViT):

```bash
python -m preprocessing.extract_mscoco_feature
```


## Acknowledgement

This code is mainly built upon [REPA](https://github.com/sihyun-yu/REPA/tree/main/preprocessing), [U-ViT](https://github.com/baofff/U-ViT), and [edm2](https://github.com/NVlabs/edm2) repository.
